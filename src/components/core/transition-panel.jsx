...
export function TabsTransitionPanel() {
  ...
  return (
    <div>
      <div className='mb-4 flex flex-wrap justify-center space-x-2 sm:space-x-4'>
        {ITEMS.map((item, index) => (
          <button
            ...
            className={`rounded-md px-2 py-1 text-xs sm:px-3 sm:py-1 sm:text-sm md:px-4 md:py-2 md:text-base font-medium ${
              activeIndex === index
      ...
      <div className='overflow-hidden border-t border-zinc-200 dark:border-zinc-700 p-2 sm:p-4 md:p-6'>
        <TransitionPanel
        ...
        >
          {ITEMS.map((item, index) => (
            <div key={index} className='py-2'>
              <h3 className='mb-2 font-medium text-zinc-800 dark:text-zinc-100 text-sm sm:text-base md:text-lg'>
                {item.subtitle}
              </h3>
              <p className='text-zinc-600 dark:text-zinc-400 text-xs sm:text-sm md:text-base'>{item.content}</p>
            </div>
          ))}
        </TransitionPanel>
      </div>
